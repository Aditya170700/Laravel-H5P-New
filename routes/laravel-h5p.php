...
Route::group(['middleware' => ['web']], function () {
    if (config('laravel-h5p.use_router') == 'EDITOR' || config('laravel-h5p.use_router') == 'ALL') {
        Route::group(['middleware' => ['auth']], function () {
            Route::get('h5p/library', "Djoudi\LaravelH5p\Http\Controllers\LibraryController@index")->name('h5p.library.index');
            Route::get('h5p/library/show/{id}', "Djoudi\LaravelH5p\Http\Controllers\LibraryController@show")->name('h5p.library.show');
            Route::post('h5p/library/store', "Djoudi\LaravelH5p\Http\Controllers\LibraryController@store")->name('h5p.library.store');
            Route::delete('h5p/library/destroy', "Djoudi\LaravelH5p\Http\Controllers\LibraryController@destroy")->name('h5p.library.destroy');
            Route::get('h5p/library/restrict', "Djoudi\LaravelH5p\Http\Controllers\LibraryController@restrict")->name('h5p.library.restrict');
            Route::post('h5p/library/clear', "Djoudi\LaravelH5p\Http\Controllers\LibraryController@clear")->name('h5p.library.clear');
        });
        ...
        Route::match(['GET', 'POST'], 'ajax/content-type-cache', 'Djoudi\LaravelH5p\Http\Controllers\AjaxController@contentTypeCache')->name('h5p.ajax.content-type-cache');
        Route::match(['GET', 'POST'], 'ajax/content-hub-metadata-cache', 'Djoudi\LaravelH5p\Http\Controllers\AjaxController@contentHubMetadataCache')->name('h5p.ajax.content-hub-metadata-cache');
